...
export default function UtangPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/debts?co_id=${id}`)
      ...
      .then(d => {
        if (Array.isArray(d)) setDebts(d.filter((x: any) => x.type === 'utang'))
  ...
  const totalUtang = debts.filter(d => !d.lunas).reduce((s, d) => s + sisa(d), 0)
  ...
  return (
    <div style={{ background: '#FFF8E1', minHeight: '100vh' }}>
      <div style={{ background: '#854F0B', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <p style={{ fontSize: 15, fontWeight: 500, color: '#FAEEDA' }}>Utang</p>
        {!isOwner && (
      ...
      <div style={{ padding: 14 }}>
        {isOwner && (
          <div style={{ background: '#FFF3CD', border: '0.5px solid #FAC775', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 13, color: '#854F0B' }}>
            Mode owner: hanya dapat melihat riwayat utang, tidak dapat menambah atau mengubah data.
          </div>
        )}
        <div style={{ background: '#fff', borderRadius: 10, border: '0.5px solid #FAC775', padding: 12, textAlign: 'center', marginBottom: 14 }}>
          ...
          {loading ? (
            ...
            return (
              <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', borderBottom: '0.5px solid #FFF3CD', cursor: 'pointer' }} onClick={() => router.push(`/utang/${d.id}`)}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: d.lunas ? '#639922' : '#D85A30', flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ height: 4, background: '#FFF3CD', borderRadius: 99, marginTop: 5, width: 120, overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: 99, width: `${p}%`, background: d.lunas ? '#639922' : '#FFC107' }} />
                  </div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 500, color: '#993C1D' }}>{fmt(d.total)}</p>
