...
// Tipe akun yang relevan untuk uang KELUAR
const TIPE_KELUAR = ['kas', 'bank', 'kewajiban', 'beban', 'hpp', 'aset_tetap']
...
export default function FormKeluarPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/accounts?co_id=${id}`)
      .then(r => r.json())
      .then(d => setAccounts((d || []).filter((a: Account) => TIPE_KELUAR.includes(a.tipe))))
  }, [router])
  ...
  return (
    <div style={{ background: '#FFF8E1', minHeight: '100vh' }}>
      <div style={{ background: '#993C1D', padding: '12px 16px' }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '0.5px solid #FAC775', padding: 18 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input value={ket} onChange={e => setKet(e.target.value)} placeholder="Beli bahan baku, bayar gaji, cicil hutang..." />
          </div>
